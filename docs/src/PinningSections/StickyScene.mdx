...
Since `<StickyScene>` is just a `<Section>` that helps you achieve the sticky scrolling effect with ease, it provides the same information like `scrolledRatio` as [Section](_/scroll_tracking/section#properties_).
