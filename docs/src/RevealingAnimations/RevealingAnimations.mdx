...
`useIntersectingTrigger` is particulary well-suited for the animations triggered when the elements are scrolled into the viewport.
...
Then, you can know whether a component is inside the viewport by giving the `ref` of a component to `useIntersectingTrigger` hook.
...
<Props of={useIntersectingTrigger} />
